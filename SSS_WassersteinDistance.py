...
def get_SSS_wasserstein_distance(model_name, dataset_path):
    ...
    for key in dyn_emb_target_pairs.keys():
        ...
        for pro_stereo, anti_stereo in targ_emb:
            ...
            while(b<10):
                ...
                for j in range(0, len(cur_dim_vec), 2):
                    if(j+1 < len(cur_dim_vec)):
                        temp_sup = np.dot(pro_stereo, np.squeeze(cur_dim_vec[j+1]))
                        superpos = np.vdot(cur_dim_vec[j], temp_sup)

                        mag = np.abs(superpos)
                        theta = np.angle(superpos)

                        interference = 2*alpha*beta*np.cos(theta)*mag

                        psi_A_prime = pro_stereo @ np.squeeze(cur_dim_vec[j]) #P_x and Psi_A
                        psi_B_prime = pro_stereo @ np.squeeze(cur_dim_vec[j+1]) #P_x and Psi_B

                        alpha_psi_A_prime = alpha*psi_A_prime
                        beta_psi_B_prime = beta*psi_B_prime

                        norm_A = np.linalg.norm(alpha_psi_A_prime)**2
                        norm_B = np.linalg.norm(beta_psi_B_prime)**2

                        P_x = norm_A + norm_B + interference

                        pro_scores.append(P_x)

                #Anti Stereotype Spectrum
                anti_scores = []
                for j in range(0, len(cur_dim_vec), 2):
                    if(j+1 < len(cur_dim_vec)):
                        temp_sup = np.dot(anti_stereo, np.squeeze(cur_dim_vec[j+1]))
                        superpos = np.vdot(cur_dim_vec[j], temp_sup)
                        ...
                        psi_A_prime = anti_stereo @ np.squeeze(cur_dim_vec[j]) #P_x and Psi_A
                        psi_B_prime = anti_stereo @ np.squeeze(cur_dim_vec[j+1]) #P_x and Psi_B
            ...
            temp_wasserstein_scores.append(wasserstein_distance(mean_pro, mean_anti))
            inst += 1

        final_scores[key] = np.mean(temp_wasserstein_scores)
        final_inst[key] = inst
    
    total_weighted_distance = sum(final_scores[pair] * final_inst[pair] for pair in final_scores)
    total_sentences = sum(final_inst[pair] for pair in final_inst)

    overall_score = total_weighted_distance / total_sentences
    overall_score

    print(f"SSS Wasserstein distance for {model_name} is: {str(overall_score)}")
